...
class AmazonComputerDataset(Amazon):
    def __init__(self, root, name='Computers', top_k_pct=0.30, threshold=0.1, train_mask=None, test_size=0.2, seed=42):
        pre_transform = T.Compose([
            # T.NormalizeFeatures(),
            T.ToUndirected(),
            # Short-range settings
            WaveGCSpectralTransform(mode='short', top_k_pct=top_k_pct, threshold=threshold),
